...
// Execute with tracking and security validation
function executeWrapped(type, args, originalFn) {
  const command = args[0];
  
  // Validate command security before execution
  const validation = validateCommand(command, args[1] || []);
  if (!validation.allowed) {
    auditLogger.logCommandExecution(command, args[1], validation);
    const error = new Error(`Command blocked: ${validation.reason}`);
    error.code = 'SECURITY_BLOCK';
    throw error;
  }
  
  // Use sanitized arguments
  if (validation.sanitizedArgs && validation.sanitizedArgs.length > 0) {
    args[1] = validation.sanitizedArgs;
  }
