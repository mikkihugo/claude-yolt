...
import { getIsDocker, hasInternetAccess } from './security/permission-manager.js';
...
// Modify Claude CLI for YOLT with secure permission management
export function modifyClaudeForYolo(originalCliPath, yoloCliPath, isLocal) {
  ...
  // SECURITY FIX: Replace dangerous permission bypass with secure calls
  // Instead of hardcoding 'true', use our secure permission manager
  auditLogger.logPermissionBypass('docker_check_replacement', {
    originalPath: originalCliPath,
    yoloPath: yoloCliPath
  });
  
  // Replace getIsDocker() calls with secure permission check
  cliContent = cliContent.replace(
    /[a-zA-Z0-9_]*\.getIsDocker\(\)/g, 
    `require('${path.resolve(__dirname, 'security/permission-manager.js')}').getIsDocker()`
  );
  debug("Replaced getIsDocker() calls with secure permission manager");
  
  // Replace hasInternetAccess() calls with secure permission check
  cliContent = cliContent.replace(
    /[a-zA-Z0-9_]*\.hasInternetAccess\(\)/g, 
    `require('${path.resolve(__dirname, 'security/permission-manager.js')}').hasInternetAccess()`
  );
  debug("Replaced hasInternetAccess() calls with secure permission manager");
  ...
  const yoloSuffixes = [
    ...
    ` ${BOLD}\x1b[35m(yolt engaged)${RESET}`,
...
export async function checkForClaudeUpdates() {
  try {
    ...
    if (currentVersion !== "latest" && currentVersion !== latestVersion) {
      ...
      // Save YOLO CLI files before update
      const { claudeDir, yoloCliPath } = findClaudeInstallation();
      let savedYoloContent = null;
      if (fs.existsSync(yoloCliPath)) {
        debug("Saving YOLO modifications before update...");
        savedYoloContent = fs.readFileSync(yoloCliPath, 'utf8');
      }
      ...
      // Restore YOLO CLI if it was saved
      if (savedYoloContent && fs.existsSync(claudeDir)) {
        debug("Restoring YOLO modifications after update...");
        fs.writeFileSync(yoloCliPath, savedYoloContent);
      }
    }
  } catch (error) {
