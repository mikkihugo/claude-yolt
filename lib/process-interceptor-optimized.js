...
export async function injectOptimizedInterceptor() {
  ...
  if (!cp._optimizedOriginals) {
    try {
      ...
      // If we can't modify cp directly, use a WeakMap
      if (!global._claudeYoltOriginals) {
        global._claudeYoltOriginals = new WeakMap();
      }
      if (!global._claudeYoltOriginals.has(cp)) {
        global._claudeYoltOriginals.set(cp, {
          spawn: cp.spawn,
          exec: cp.exec
        });
      }
  ...
  // Replace with optimized versions (handle read-only properties)
  try {
    cp.spawn = optimizedSpawn;
    cp.exec = optimizedExec;
  } catch (e) {
    // If properties are read-only, use Object.defineProperty
    try {
      Object.defineProperty(cp, 'spawn', {
        value: optimizedSpawn,
        writable: true,
        configurable: true
      });
      Object.defineProperty(cp, 'exec', {
        value: optimizedExec,
        writable: true,
        configurable: true
      });
    } catch (e2) {
      console.warn('⚠️  Could not intercept child_process - using fallback mode');
      return;
    }
  }
